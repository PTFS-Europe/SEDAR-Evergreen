...
sub create_borrower {
    ...
    my $bor  = Fieldmapper::actor::user->new();
    ...
    $bor->usrname($barcode);
    $bor->passwd('xxxxx')
      ;    # Hard coded default password to be changed by the user
    ...
    $dob_m =~ tr/ /0/;
    my $dob =
      $cgi->param('dob_y') . "-" . $dob_m . "-" . $dob_d . "T00:00:00+0000";
    $bor->dob($dob);
    $bor->addresses( [$addr] );
    $bor->cards(     [$card] );
    $bor->isnew(1);
    ...
    my $auth = login( '<user>', '<password>' )
      ; # Set to user and password of user with permissions to create a new patron

    my $newbor =
      $U->simple_scalar_request( "open-ils.actor",
...
# URL of OPAC login screen (e.g. https://prd1.sedar.org.uk/opac/en-GB/skin/stirling/xml/myopac.xml?d=0)
    my $message =
'Your new library account has been successfully created. Your login details are:
...
If you do not collect your library card within two months, your account will be deactivated.';
    sendMail( $cgi->param('email'), '<library_email>',
        '<library_name> account confirmation', $message )
      ;    # Specify the library email address and name
    $message = 'An online account was created by the following user:

Name:        ' . $cgi->param('fname') . ' ' . $cgi->param('surname') . '
Barcode:     ' . $barcode;
    sendMail( '<to_email_address>', '<from_email_address>',
        '<library_name> account creation', $message )
      ;    # Specify the to and from addresses, and the Library name
...
sub login {
    ...
    my $seed = $U->simple_scalar_request( 'open-ils.auth',
...
sub get_sedar_branch_list {
    my $branch_list;
    return $branch_list = $U->simple_scalar_request(
        "open-ils.cstore",
...
sub get_last_barcode {
    ...
    return $last_id = $U->simple_scalar_request(
        "open-ils.cstore",
