...
__PACKAGE__->register_method(
        ...
        signature       =>'Check barcode exists and is current - returns user ID and t or f flag for active barcodes'
);
...
sub barcode_current {
        ...
        if (@{$card})
        {
            my $return={usr=>$card->[0]->usr,active=>$card->[0]->active};
            return $return;
        }
        else
        {
...
sub sedar_nec_card_check {
    ...
    {
        my $return={sedar_card_flag=>'badscan'};
        return $return;
    }
    my $barcode_current=check_barcode_current($auth,$borrower->{nec_number});
    #return $barcode_current;
    my $usr=0;
    my $active=0;
    if (ref($barcode_current) eq "HASH")
    {
        ...
        if ($active eq 't')
        {
            ...
            if (ref($fleshed_user->billing_address) eq 'Fieldmapper::actor::user_address')
            {
                $db_billing_street1=$fleshed_user->billing_address->street1;
                $db_billing_postcode=$fleshed_user->billing_address->post_code;
            }
            if (ref($fleshed_user->mailing_address) eq 'Fieldmapper::actor::user_address')
            {
                $db_mailing_street1=$fleshed_user->mailing_address->street1;
                $db_mailing_postcode=$fleshed_user->mailing_address->post_code;
            }

            # check first_given_name
            if (lc($borrower->{first_given_name}) ne lc($fleshed_user->first_given_name))
            {
                $fgn_chg='t';
            }
            # check family_name
            ...
            # check overall match
            if ($fgn_chg eq 't' || $fn_chg eq 't' || $dob_chg eq 't' || $addr_chg eq 't' || $pc_chg eq 't' || $phone_chg eq 't')
            {
                my $db_borrower={
                ...
                my $return={sedar_card_flag=>'found_detail_change',userid=>$usr,borrower=>$borrower,db_borrower=>$db_borrower,db_fleshed=>$fleshed_user,upd_addr=>$upd_addr};
                return $return;
            }
            else
            {
                #card data matches - load borrower as normal
                my $return={sedar_card_flag=>'found',userid=>$usr,barcode=>$borrower->{nec_number}};
                return $return;
            }
        }
        elsif ($active eq 'f')
        {
            ...
            my $return={sedar_card_flag=>'found_old_card',userid=>$usr,barcode=>$borrower->{nec_number}};
            return $return;
        }
    }
    else
    {
        ...
        if ($patron_search_matches == 0)
        {
            ...
            my $return={sedar_card_flag=>'not_found_add',borrower=>$borrower};
            return $return;
        }
        elsif ($patron_search_matches > 0)
        {
            # matches found - return search hash
            my $return={sedar_card_flag=>'not_found_matches',search_hash=>$borsearch,nec_number=>$borrower->{nec_number}};
            return $return;
        }
    }
    ...
    sub build_borrower_from_nec_card {
        ...
        if ($card_array[3])
        {
            $card_array[3]=substr($card_array[3],4,4)."-".substr($card_array[3],2,2)."-".substr($card_array[3],0,2);
        }
    ...
    sub check_barcode_current {
        ...
        ->request( 'open-ils.actor.sedar.barcode.current',
            $auth,
            $barcode)->gather;
        return $response;
    }
    ...
    sub patron_search {
        ...
        my $response=OpenSRF::AppSession
        ->create('open-ils.actor')
        ->request('open-ils.actor.patron.search.advanced',
            $auth,
            $search)->gather;
